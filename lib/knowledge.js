 ...
 */
import { internalMarkdownBoldToHtml } from './textUtils.js';
...
RESPONSE SLA & DESK AVAILABILITY — Docket **Websites** team (this chat + websites@yourdocket.com)
- Live desk hours (US Mountain Time, America/Denver): Monday through Friday, 7:00 AM – 5:00 PM.
- Closed Saturdays, Sundays, and US federal holidays (server maintains a dated list for escalation logic).
- **After hours / weekends / holidays:** Be clear proactive human/email follow-ups begin the **next business day** once the desk reopens — do not promise overnight or same-day team coverage.
- Internal Slack notifications from escalations may still fire off-hours, but **client expectation** remains next business day for human follow-up unless you know otherwise.
...
6.4 Help Resources
This support chat is the primary place for editing guidance: clarify what they're trying to do, walk through Elementor basics when helpful, suggest alternatives before handing off—and when the Websites team must intervene, collect their details and escalate from here (same flow as scripted requests previously sent to legacy forms).
Website Review Help: https://dockethosting.com/website-review-help/ - Post-review next steps and FAQ.
Support Email: websites@yourdocket.com — answered during **published desk hours** (Mon–Fri, 7:00 AM–5:00 PM US Mountain Time), excluding weekends and US federal holidays. Outside those windows, replies begin the **next business day**.
...
Clients cannot install plugins. To add tracking scripts, chatbots, Google Analytics, Meta Pixel, or other code to their site, clients should use the Code Snippets plugin (if they have access). If that is not enough or they lack access, continue in this chat: understand the goal, try practical alternatives, collect details if escalation is needed—the team adds scripts that cannot be handled through Code Snippets.
...
GA is not automatically added to standard (self-managed) websites. For WebsiteVIP clients, GA is added in Week 1. For self-managed clients who want GA, handle it here: capture their GA tracking code/script details and escalate to the Websites team for placement when needed.
10.3 Google Search Console
Clients set up Search Console in their own Google account. When Search Console asks for a site verification meta tag, collect the tag in this conversation and escalate to the Websites team—they add it to the site.
10.4 Rank Math SEO
...
To deactivate a Docket website: handle it in this chat—gather the client's details and escalate to the Websites team. Cancellation requests are submitted to the finance team for processing.
...
Clients asking us to make edits. For self-managed sites: the client makes their own edits via Elementor—coach them briefly in chat if stuck; escalate if backend or team action is needed. For WebsiteVIP clients: collect details and escalate the edit request to the team.
...
- Final Signoff = We've sent the client a final approval email and are waiting for them to give the go-ahead to push the site live

TRELLO PROJECT LOOKUP
The project lookup tool only shows websites currently in the build pipeline. If no project is found, it most likely means the client's website is already built and live — NOT that they don't exist. Don't tell the client you can't find them. If they're asking about build status and the lookup returns nothing, let them know their site may already be live and ask them to confirm their website URL.
...
- Client receives a final approval email — we're waiting for their go-ahead to push the site live (Trello label: Final Signoff)
...
- Answer editing questions and guide Elementor steps in this chat when helpful; escalate when the Websites team must make the change
...
AUTOMATED EMAILS
All automated emails come from websites@yourdocket.com. These emails are system-generated and always go out — if a client says they didn't receive one, they should:
- Check their spam/junk folder
- Search their inbox for "websites@yourdocket.com"
- Check any email filters that may have caught it
- Add websites@yourdocket.com to their contacts to prevent future filtering

Common automated emails clients receive:
- Schedule Your Website Review (when their draft is ready)
- Website Review Link (with 72-hour review window)
- Review Follow-up (if they haven't completed their review)
- Edits Complete notification (with link to check changes)
- Login Credentials (sent ~5 minutes after launch)
- Directory Listings report (Pro/ServiceCore clients)

These emails definitely go out. If a client says they didn't get one, it's almost always a spam filter issue.
...
13.5 Support Email
websites@yourdocket.com — primary channel for **website** issues. Desk hours: Monday–Friday, 7:00 AM – 5:00 PM US Mountain Time (America/Denver). Closed weekends and US federal holidays. After hours: expect **next business day** follow-up by email; do not promise same-day human coverage.
...
After launch, standard clients are responsible for their own site. We built it, launched it, and handed it off. Support them in this chat first; escalate when the team needs to act. If they want ongoing management, that's WebsiteVIP at $299/month.
...
export const KNOWLEDGE_BASE = internalMarkdownBoldToHtml(KNOWLEDGE_BASE_RAW);
