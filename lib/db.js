...
/**
 * Persist client contact fields (merge with existing row).
 * Email replaces client_email when a valid-ish value is provided.
 */
export async function updateConversationContact(conversationId, { fullName, businessName, email } = {}) {
  const trim = v => {
  ...
  const fn = trim(fullName);
  const biz = trim(businessName);
  const emRaw = trim(email);
  const em = emRaw ? emRaw.toLowerCase() : null;

  const mergedName = fn ?? row.contact_full_name ?? null;
  const mergedBusiness = biz ?? row.business_name ?? null;
  const mergedEmail = em || row.client_email || 'anonymous';

  const result = await sql`
    UPDATE conversations
    SET contact_full_name = ${mergedName},
        business_name = ${mergedBusiness},
        client_email = ${mergedEmail},
        updated_at = NOW()
    WHERE id = ${conversationId}
    RETURNING *
  `;
  ...
  if (!result || result.length === 0) {
    throw new Error('Failed to update conversation contact');
  }
