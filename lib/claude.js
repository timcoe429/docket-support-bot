...
import { sendSlackEscalation, formatConversationTranscriptPlain } from './slack.js';
import { getConversation, getConversationMessages, updateConversationContact } from './db.js';
...
Before you call escalate_to_team, you must have the client's <strong>full name</strong>, <strong>business name</strong>, and <strong>best email address for follow-up</strong> — ideally from their contact strip in the widget, otherwise ask in chat and wait for answers. Never guess or invent contact details.
...
const TOOLS = [
    ...
    },
    {
        ...
        description:
            'Notify the Docket Websites team that a client needs human help. ONLY call after full name, business name, AND email have been captured (contact card or explicitly in chat). Use for issues that require a human (DNS, installs, outages, unclear cases) or when they insist on a person — after explaining email-only follow-up.',
        input_schema: {
            type: "object",
            properties: {
                reason: {
                ...
                contact_email: {
                    type: 'string',
                    description: 'Valid email address they use for replies — verify spelling if unclear'
                }
            },
            ...
            try {
                ...
                if (missing.length) {
                    return JSON.stringify({
                        ...
                        guidance:
                            `Cannot notify the team yet — still need ${missing.join(', ')}. Ask the client politely. If they're on mobile, remind them they can fill the contact fields above the chat. Explain we follow up by email only once the team jumps in.`
                    });
                }
                ...
                const { preview } = formatConversationTranscriptPlain(rows);
