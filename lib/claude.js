...
import { normalizeAssistantChatHtml } from './textUtils.js';
...
CRITICAL — The widget renders your reply as HTML. Plain newlines collapse into an ugly wall of text. Asterisk Markdown is NEVER supported — if you write **bold** it appears broken.

Formatting rules every time:
- Break up ideas with <br><br>; use <ul><li>...</li></ul> for multiple questions — never cram three questions inline.
- Emphasis only with HTML: <strong>...</strong>; never ** or ##
- Steps: <ol><li>...</li></ol>; links: <a href="..." target="_blank">
...
export async function generateResponse(userMessage, conversationHistory = [], context = {}) {
    ...
    try {
        ...
        const responseText = normalizeAssistantChatHtml(textBlocks.map(block => block.text).join('\n\n'));
